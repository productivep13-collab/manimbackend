...
def generate_voice_audio_with_fallback(text: str, out_path: str):
    if ELEVEN_KEY:
        try:
    ...
    fallback_dur = max(1.0, min(3.0, len(text.split()) / 2.5))
...
def generate_all_tts_parallel(segments, tmpdir, max_workers=6):
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in as_completed(future_to_index):
            ...
            if err:
                print(f"[WARN] TTS failed for segment {idx+1}")
                dur = float(segments[idx].get("duration", 3.0))
                run_cmd(["ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=22050:cl=mono",
                        "-t", f"{dur:.2f}", outp], timeout=15)
...
def generate_manim_scene_adaptive(segments: list) -> str:
    """Generate Manim scenes with progressive building (NO screen clearing between steps)"""
...
class GeneratedScene(Scene):
    def construct(self):
        ...
        # Position tracking for continuous flow
        self.current_y = 3.0
...
"""]
    ...
    for i, seg in enumerate(segments):
        segment_id = i + 1
        duration = seg.get("actual_duration", seg.get("duration", 5.0))
        ...
        scene_code.append(f"""
        # === SEGMENT {segment_id}: {layout.upper()} ===
""")
        
        # CLEAR only for title segments after intro
        if layout == "title" and i > 0:
            scene_code.append("""
        # New section - clear screen
        self.play(*[FadeOut(m) for m in self.mobjects], run_time=0.5)
        self.current_y = 3.0
        self.wait(0.2)
""")
        
        # === CALCULATION: Show work step-by-step ===
        if layout == "calculation" and step_data.get("calculation_steps"):
            ...
            scene_code.append(f"""
        # Work through calculation progressively
        step_time = {duration / max(len(steps), 1):.2f}
        
""")
            
            for j, step_text in enumerate(steps):
                safe_step = sanitize_for_text(step_text)
                safe_note = sanitize_for_text(notes[j]) if j < len(notes) and notes[j] else ""
                
                scene_code.append(f"""
        # Step {j+1}
        step_{j} = Text("{safe_step}", font_size=32, color=WHITE)
        step_{j}.move_to([0, self.current_y, 0], aligned_edge=LEFT)
        step_{j}.shift(LEFT*5)
""")
        ...
        self.play(Write(step_{j}), FadeIn(note_{j}, shift=RIGHT*0.2), run_time=step_time*0.6)
""")
                else:
                    scene_code.append(f"""
        self.play(Write(step_{j}), run_time=step_time*0.6)
""")
...
""")
        ...
        # === STEP: Add calculation step ===
        elif layout == "step":
            scene_code.append(f"""
        # Add step to workspace
        label = Text("Step {segment_id}", font_size=24, color="{color}", weight=BOLD)
        label.move_to([0, self.current_y, 0], aligned_edge=LEFT)
        label.shift(LEFT*5.5)
        
        content = Text("{display_text}", font_size=30, color=WHITE)
        content.next_to(label, DOWN, buff=0.25, aligned_edge=LEFT)
        
        self.play(FadeIn(label, shift=DOWN*0.15), run_time=0.25)
        self.play(Write(content), run_time={duration*0.6:.2f})
        self.wait({duration*0.4:.2f})
...
""")
        ...
        self.play(Write(title), run_time={duration*0.5:.2f})
        self.play(Create(line), run_time=0.4)
        self.wait({duration*0.3:.2f})
...
""")
        ...
        self.play(Write(header), Create(line), run_time=0.7)
        self.current_y = 2.2
        self.wait({duration-0.7:.2f})
""")
        ...
        self.play(Create(box), Write(eq), run_time={duration-0.3:.2f})
...
""")
        ...
        self.play(Create(shape), run_time={duration-0.4:.2f})
        self.wait(0.3)
""")
        ...
        text.move_to([0, self.current_y, 0], aligned_edge=LEFT)
        text.shift(LEFT*5)
        ...
        self.play(FadeIn(text, shift=RIGHT*0.2), Create(circles), run_time={duration*0.7:.2f})
        self.current_y -= 1.2
        self.wait({duration*0.3:.2f})
""")
        ...
        ex_label.move_to([0, self.current_y, 0], aligned_edge=LEFT)
        ex_label.shift(LEFT*5.5)
        ...
        self.play(FadeIn(ex_text, shift=DOWN*0.2), run_time={duration-0.3:.2f})
...
""")
        ...
        self.play(Write(text), run_time={duration*0.7:.2f})
        self.current_y -= 0.9
        self.wait({duration*0.3:.2f})
""")
        
        # Auto-scroll if screen is getting full
        if segment_id > 0 and segment_id % 7 == 0:
            scene_code.append("""
        # Gentle fade of older content if screen is full
